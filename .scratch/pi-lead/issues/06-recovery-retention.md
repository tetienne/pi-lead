# 06: Recover interrupted work and retain useful diagnostics
...
- [ ] Exercise durable host-owned records and ownership reconciliation across VM, Pi, Herdr and Git state.
- [ ] Detect uncertain prompts/publication and stale/replaced identities; do not replay side effects merely because a response was lost.
- [ ] Confirm worker termination; preserve stopped diagnostic tabs/artifacts; never invoke Herdr's host-Pi restore for guest sessions.
- [ ] Enforce seven-day successful-log retention and explicit clearing for failure diagnostics without deleting delivered branches/commits.
- [ ] Cleanup failure blocks DONE and remains actionable after restart.
...
Test public task lifecycle and policy behavior at the approved seams; use controlled runtime substitutes and supplied judgments where appropriate, then real integrations for behavior that substitutes cannot prove. Inject failures before and after prompt delivery, artifact collection, commit/push acknowledgement and teardown. Reconcile after restart, verify no duplicate effects or automatic resumption, retain failed diagnostics and expire successful logs using a controlled clock.
