# 16: Verify reusable installation and the complete supported experience

**What to build:** a fresh consuming project can install a pinned package and complete the approved end-to-end workflows with documented operating limits on every supported host target.
...
- [ ] Run combined lifecycle, isolation, provider, review, publication, routing-budget, scheduler and recovery scenarios against the final versions.
- [ ] Document measured startup and any supported-version limits; publish only claims supported by actual acceptance evidence.
