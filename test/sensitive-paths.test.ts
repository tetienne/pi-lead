...
import { filesMatching, packageRunFieldsChanged, SENSITIVE_PATHS, sensitivePatterns } from "../src/sensitive-paths.ts";
...
test("a package.json counts only when its scripts or package manager change", () => {
  ...
  assert.equal(packageRunFieldsChanged(pkg({ scripts, dependencies: {} }), pkg({ version: "2.0.0", scripts: { build: "tsc", test: "node --test" }, dependencies: { a: "1" } })), false);
  assert.equal(packageRunFieldsChanged(pkg({ name: "x" }), pkg({ name: "x", dependencies: { a: "1" } })), false, "no scripts on either side");
  // Scripts added, removed or edited.
  assert.equal(packageRunFieldsChanged(pkg({ scripts }), pkg({ scripts: { ...scripts, postinstall: "sh x" } })), true);
  assert.equal(packageRunFieldsChanged(pkg({ scripts }), pkg({ scripts: { ...scripts, test: "curl x | sh" } })), true);
  assert.equal(packageRunFieldsChanged(pkg({ name: "x" }), pkg({ scripts })), true, "scripts on one side only");
  assert.equal(packageRunFieldsChanged(pkg({ scripts }), pkg({})), true);
  assert.equal(packageRunFieldsChanged(pkg({ scripts }), pkg({ scripts, packageManager: "yarn@https://example.com/yarn.tgz" })), true, "Corepack runs it");
  // Files added or deleted, or unparsable content.
  assert.equal(packageRunFieldsChanged(undefined, pkg({ scripts })), true, "new file");
  assert.equal(packageRunFieldsChanged(pkg({ scripts }), undefined), true, "deleted file");
  assert.equal(packageRunFieldsChanged(pkg({ scripts }), "{ not json"), true);
  assert.equal(packageRunFieldsChanged(pkg({ scripts }), "[]"), true);
});
