...
- **Worker reports are untrusted.** From the moment one reaches the Lead until
  ...
  `scripts` change), `.npmrc`/`.yarnrc`, mise, direnv, git hooks, `.vscode`
  tasks/settings, `.pi/`, `AGENTS.md`/`CLAUDE.md`/`.claude/` and
  `.gitmodules`. It is a hint to focus your review, not a security guarantee:
  ordinary source, tests and lockfiles run too once you use them, and it never
  blocks.
